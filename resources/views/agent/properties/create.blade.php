...
<head>
    ...
    <style>
        * { font-family: 'Montserrat', sans-serif; }
        ...
        #map { height: 300px; border-radius: 12px; border: 1.5px solid #e2e6f0; }
    </style>
</head>
...
<nav style="position:fixed;top:0;width:100%;z-index:50;background:#fff;border-bottom:1px solid #e2e6f0;box-shadow:0 1px 4px rgba(0,0,0,.06);">
    <div style="max-width:860px;margin:0 auto;padding:12px 24px;display:flex;align-items:center;gap:12px;">
        ...
        <span style="font-family:'Cinzel',serif;color:#004370;font-weight:600;font-size:13px;letter-spacing:.04em;">Nueva Propiedad</span>
    </div>
</nav>
...
<main style="max-width:860px;margin:0 auto;padding:88px 24px 80px;">
    ...
    <form method="POST" action="{{ route('agent.properties.store') }}" enctype="multipart/form-data" id="property-form">
        @csrf
            ...
            <div style="display:flex;flex-direction:column;gap:16px;">
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                              ...
                              class="inp" style="resize:none;">{{ old('description') }}</textarea>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;">
                    <div>
                        <label class="lbl">Precio <span style="color:#ef4444;text-transform:none;">*</span></label>
                        <input type="number" name="price" value="{{ old('price') }}" required min="0" step="0.01" placeholder="0" class="inp">
                    </div>
                    <div>
        ...
        <div class="card">
            ...
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:20px;">
                <div id="bedrooms-field">
                    ...
                    <input type="number" name="bedrooms" value="{{ old('bedrooms') }}" min="0" class="inp">
                </div>
                <div id="bathrooms-field">
                    <label class="lbl">🚿 Baños</label>
                    <input type="number" name="bathrooms" value="{{ old('bathrooms') }}" min="0" step="0.5" class="inp">
                </div>
                <div>
                ...
                <div style="grid-column:span 2;">
                    ...
                    <input type="number" name="land_area" value="{{ old('land_area') }}" min="0" step="0.01" class="inp">
                </div>
            </div>
            ...
            <div>
                ...
                <div id="features-container" style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:10px;min-height:32px;">
                    ...
                    <div class="feature-tag" style="display:flex;align-items:center;gap:6px;background:rgba(0,67,112,.08);color:#004370;font-size:12px;font-weight:600;padding:5px 12px;border-radius:99px;">
                        ...
                        <span>{{ $feat }}</span>
                        <button type="button" onclick="this.parentElement.remove()" style="display:flex;background:none;border:none;cursor:pointer;color:inherit;padding:0;opacity:.6;" onmouseover="this.style.opacity=1;this.style.color='#ef4444'" onmouseout="this.style.opacity=.6;this.style.color='inherit'">
                ...
                <div style="display:flex;gap:8px;">
                    <input type="text" id="feature-input" placeholder="Ej. Alberca, Terraza, A/C, Vista al mar…"
                           class="inp" style="flex:1;">
                    <button type="button" onclick="addFeature()"
                            style="display:flex;align-items:center;gap:6px;padding:10px 16px;background:#f0f4ff;color:#004370;border:1.5px solid #c1c7d1;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;transition:background .15s;white-space:nowrap;"
        ...
        <div class="card">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                ...
                <div>
                    ...
                    <input type="text" name="city" value="{{ old('city','Mazatlán') }}" class="inp">
                </div>
                <div>
                    <label class="lbl">Estado</label>
                    <input type="text" name="state" value="{{ old('state','Sinaloa') }}" class="inp">
                </div>
                <div style="grid-column:span 2;">
        ...
        <div class="card">
            ...
            {{-- Portada --}}
            <p style="font-size:11px;font-weight:700;color:#414750;text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px;">Foto de portada</p>
            <div id="cover-drop" class="drop-zone" onclick="document.getElementById('cover-input').click()">
                ...
                <div id="cover-preview-wrap" style="display:none;">
                    <img id="cover-preview-img" style="max-height:180px;border-radius:10px;object-fit:cover;margin:0 auto;display:block;" src="" alt="">
                    <button type="button" onclick="event.stopPropagation();removeCover()"
                        ...
                        <span class="material-symbols-outlined" style="font-size:15px;">delete</span> Quitar portada
                    </button>
                </div>
            </div>
        ...
        <div id="preventa-section" class="card" style="display:none;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <div>
                    ...
                    <input type="date" name="delivery_date" value="{{ old('delivery_date') }}" class="inp">
                </div>
                <div>
                    <label class="lbl">Avance de construcción (%)</label>
                    <input type="number" name="construction_progress" value="{{ old('construction_progress') }}" min="0" max="100" placeholder="0–100" class="inp">
                </div>
            </div>
        </div>
        ...
        <div id="vacacional-section" class="card" style="display:none;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <div>
                    ...
                    <input type="number" name="min_nights" value="{{ old('min_nights') }}" min="1" placeholder="2" class="inp">
                </div>
                <div>
                    <label class="lbl">Noches máximas</label>
                    <input type="number" name="max_nights" value="{{ old('max_nights') }}" min="1" placeholder="30" class="inp">
                </div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <textarea name="notes" rows="3"
                      ...
                      class="inp" style="resize:none;">{{ old('notes') }}</textarea>
        </div>
